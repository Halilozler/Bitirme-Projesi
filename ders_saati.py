...
def en_kucuk_bulma(Cihaz_id):
    saat = sorted(cursor.execute(f"SELECT Tarih, a.Id FROM tbl_DersSaat a, tbl_Ders b WHERE a.Durum = 1 and b.Durum = 1 and b.Id = a.Ders_Id and b.Sinif_Id = {Cihaz_id}"))
    if len(saat) == 0:
        #false_cekme(saat)
        return (now, now, now)
    bas_saati = saat[0][0]
